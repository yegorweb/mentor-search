...
let mentorship_entries: Ref<Entry[]> = ref(await entryStore.get('mentor', town.value._id, school.value._id) as any)
shuffle(mentorship_entries.value)

async function updateEntries() {
  mentorship_entries.value = await entryStore.get('mentor', town.value._id, school.value._id) as any
  shuffle(mentorship_entries.value)
}
...
<template>
  <v-container>
    ...
    >
      Записей нет, или вы на всё откликнулись
    </v-col>
  </v-container>
</template>
